...
# counter entre DONE e IDLE
tempo_exibicao_final = 60
frames_em_done = 0
...
class Application:
    ...
    def update(self):
        ...
        if ESTADOS["APLICACAO"] <= estadoAtual <= ESTADOS["FISICA"]:
            ...
            if progressoAnimacao >= 1.0:
                ...
                # remove o anel mais externo se já estamos no PC direito
                if segment_start_x == destino_x and acquired_colors:
                    acquired_colors.pop()

                estadoAtual += 1          # avança para o próximo estado
                segment_start_x, segment_start_y = mensagem_x, mensagem_y

        # ------------------------------------------------------------
        #  Estado final: exibe “Oi!” por alguns frames e decide
        # ------------------------------------------------------------
        elif estadoAtual == ESTADOS["DONE"]:
            frames_em_done += 1
            if frames_em_done >= tempo_exibicao_final:
                frames_em_done = 0        # zera o contador

                if loop_anim:
                    # reinicia imediatamente (volta a APPLICACAO)
                    mensagem_x       = mensagem_start_x
                    mensagem_y       = y_positions[4]
                    mensagem_angulo  = 0.0
                    acquired_colors  = [VERDE]
                    ...
                    # volta ao repouso e espera novo ESPAÇO
                    reset_estado()
    ...
    def render(self):
        ...
        if estadoAtual in (
            ...
            ESTADOS["ENLACE"], ESTADOS["FISICA"], ESTADOS["MOVE"],
            ESTADOS["DFISICA"], ESTADOS["DENLACE"], ESTADOS["DREDE"],
            ESTADOS["DTRANSPORTE"], ESTADOS["DONE"]
        ):
            scale_factor = 1.0
            # trecho horizontal = mesmo y da Física e x variando
            if (
                estadoAtual == ESTADOS["MOVE"]
                and segment_start_y == y_positions[0]
                and mensagem_y == y_positions[0]
                and segment_start_x != destino_x
            ):
                scale_factor = 1.0 - 0.5 * progressoAnimacao  # 1 → 0.5

            self.renderer.desenha_mensagem(
                mensagem_x, mensagem_y, acquired_colors,
                rot=mensagem_angulo,
        ...
        if current_msg:
            ...
            if estadoAtual == ESTADOS["IDLE"]:
                self.renderer.escreve_texto(
                    200, 500, "Pressione ESPACO para iniciar a animacao")
                self.renderer.escreve_texto(
                    150, 470, "Visualização do encapsulamento de pacotes")

        # “Oi!” no início e no fim
    ...
    def key_callback(self, window, key, scancode, action, mods):
        ...
        elif key == glfw.KEY_L:          # tecla L = liga/desliga loop
            global loop_anim
            loop_anim = not loop_anim
